...
export function normalizeProfileUsername(service: ProfileService, input: string): string {
  ...
  if (!USERNAME_PATTERN.test(candidate)) {
    throw new Error(`Enter a valid ${service === 'letterboxd' ? 'Letterboxd' : 'TV Time'} username`);
  }
